...
import com.erp.wms.material.inventory_putaway.view.MainListView;
...
public class MainActivity extends SherlockActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String title = "Inventory Putaway";
    ...
    private void setMainViewRecords(int page) {
        MaterialInventoryPutawayAPI.GetHeaders(page, 0, 0, 0, 0, new RestfulResponse(this) {
            @Override
                ...
                try {
                    ...
                    for(int jrecord_idx = 0; jrecord_idx < jrecords.length(); jrecord_idx++) {
                        ...
                        if (!record.isNull("code"))
                            recordData.setPutawayCode(record.getString("code"));
                        if (!record.isNull("putaway_date"))
                            recordData.setPutawayDate(DateTimeUtil.FromDateString(record.getString("putaway_date")));
						if (!record.isNull("pallet"))
                            recordData.setPallet(record.getInt("pallet"));
    ...
    private void showHeaderActivity() {
        ...
        intent.putExtra("title", "Create Putaway");
        this.startActivityForResult(intent, 1);
    }

    private void showEnrollActivity(long m_inventory_putaway_id) {
        Intent intent = new Intent(this, EnrollActivity.class);
        intent.putExtra("title", "Enroll Putaway");
        intent.putExtra("m_inventory_putaway_id", m_inventory_putaway_id);
        this.startActivityForResult(intent, 2);
    }

    private void showDetailActivity(long m_inventory_putaway_id) {
        Intent intent = new Intent(this, ViewActivity.class);
        intent.putExtra("title", "Putaway Detail");
        intent.putExtra("m_inventory_putaway_id", m_inventory_putaway_id);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                case 1:
                    if (extra != null) {
                        long m_inventory_putaway_id = extra.getLong("m_inventory_putaway_id");
                        showEnrollActivity(m_inventory_putaway_id);
                    }
