...
public class HeaderFormView extends ScrollView {
    ...
    public HeaderFormView(Context context) {
        ...
        scrollViewContainer.setOrientation(LinearLayout.VERTICAL);
            ...
            TextView lblType = new TextView(mContext);
            lblType.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
            lblType.setText("Type");
            lblType.setPadding(0, (int) context.getResources().getDimension(R.dimen.layout_form_field_padding_top), 0, 0);
            scrollViewContainer.addView(lblType);

            String types[] = {"HANDLING", "CUSTOM", "PO", "DEFECT", "STOCKTAKE", "OTHER"};
            ArrayAdapter<String> spTypeAdapter = new ArrayAdapter<>(mContext, android.R.layout.simple_spinner_item, types);
            spTypeAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);

            mSpType = new Spinner(mContext);
            mSpType.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
            mSpType.setAdapter(spTypeAdapter);
            scrollViewContainer.addView(mSpType);
    ...
    public HeaderModel getData() {
        ...
        data.setAdjustCode(mEtNo.getText().toString());
        data.setAdjustDate(mDpDate.getCalendar());
		data.setAdjustType(mSpType.getSelectedItem().toString());
    ...
    public void setData(HeaderModel headerModel) {
        mEtNo.setText(headerModel.getAdjustCode());
        mDpDate.setDate(headerModel.getAdjustDate());
		if (headerModel.getAdjustType() != null) {
            if (headerModel.getAdjustType().equals("HANDLING"))
                mSpType.setSelection(0, false);
            else if (headerModel.getAdjustType().equals("CUSTOM"))
                mSpType.setSelection(1, false);
			else if (headerModel.getAdjustType().equals("PO"))
                mSpType.setSelection(2, false);
			else if (headerModel.getAdjustType().equals("DEFECT"))
                mSpType.setSelection(3, false);
			else if (headerModel.getAdjustType().equals("STOCKTAKE"))
                mSpType.setSelection(4, false);
			else if (headerModel.getAdjustType().equals("OTHER"))
                mSpType.setSelection(5, false);
            else
                mSpType.setSelection(5, false);
        }
