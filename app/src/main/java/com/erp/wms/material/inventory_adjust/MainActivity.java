...
import com.erp.wms.material.inventory_adjust.view.MainListView;
...
public class MainActivity extends SherlockActivity {
    ...
    private void setMainViewRecords(int page) {
        MaterialInventoryAdjustAPI.GetHeaders(page, 0, 0, 0, 0, new RestfulResponse(this) {
            @Override
                ...
                try {
                    ...
                    for(int jrecord_idx = 0; jrecord_idx < jrecords.length(); jrecord_idx++) {
                        ...
                        if (!record.isNull("code"))
                            recordData.setAdjustCode(record.getString("code"));
                        if (!record.isNull("adjust_date"))
                            recordData.setAdjustDate(DateTimeUtil.FromDateString(record.getString("adjust_date")));
                        if (!record.isNull("adjust_type"))
                            recordData.setAdjustType(record.getString("adjust_type"));
    ...
    private void showHeaderActivity() {
        ...
        intent.putExtra("title", "Create Adjust");
        this.startActivityForResult(intent, 1);
    }

    private void showEnrollActivity(long m_inventory_adjust_id) {
        Intent intent = new Intent(this, EnrollActivity.class);
        intent.putExtra("title", "Enroll Adjust");
        intent.putExtra("m_inventory_adjust_id", m_inventory_adjust_id);
        this.startActivityForResult(intent, 2);
    }

    private void showDetailActivity(long m_inventory_adjust_id) {
        Intent intent = new Intent(this, ViewActivity.class);
        intent.putExtra("title", "Adjust Detail");
        intent.putExtra("m_inventory_adjust_id", m_inventory_adjust_id);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                case 1:
                    if (extra != null) {
                        long m_inventory_adjust_id = extra.getLong("m_inventory_adjust_id");
                        showEnrollActivity(m_inventory_adjust_id);
                    }
