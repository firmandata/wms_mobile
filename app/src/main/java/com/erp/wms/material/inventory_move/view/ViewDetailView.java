...
public class ViewDetailView extends MyListRecord {
    ...
    public ViewDetailView(Context context, DetailModel data) {
        ...
        setId(mData.getInventoryMoveDetailId());
    }

    @Override
    public void setId(long id) {
        super.setId(id);
        mData.setInventoryMoveDetailId(id);
    }
    ...
    protected LinearLayout firstRowFirstCol() {
        ...
        tvFrom.setText(mData.getPalletFrom() + " -> " + mData.getPalletTo());
        tlSelf.addView(tvFrom);

        // -- Second Line
        TextView tvTo = new TextView(mContext);
        tvTo.setText(mData.getGridFromCode() + " -> " + mData.getGridToCode());
        tlSelf.addView(tvTo);
