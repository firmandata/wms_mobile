...
import com.erp.wms.material.inventory_putaway.view.ViewDetailView;
import com.erp.wms.material.inventory_putaway.view.ViewHeaderView;
...
public class ViewActivity extends SherlockActivity implements ActionBar.TabListener {
    ...
    private DetailModel mInventoryPutawayDetailModel;
        ...
        mDetailView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (viewDetailView != null)
                    mInventoryPutawayDetailModel = viewDetailView.getData();
            }
        });
    ...
    private void setDetailViewRecords(int page) {
        MaterialInventoryPutawayAPI.GetDetails(page, mInventoryPutawayId, new RestfulResponse(this) {
            @Override
            public void onSuccess(org.json.JSONObject response) {
                try {
                    ...
                    for (int jrecord_idx = 0; jrecord_idx < jrecords.length(); jrecord_idx++) {
                        ...
                        DetailModel recordData = new DetailModel(mInventoryPutawayId, record.getLong("m_inventory_putawaydetail_id"));
    ...
    private void showHeaderActivity() {
        ...
        intent.putExtra("title", "Edit Putaway");
        intent.putExtra("m_inventory_putaway_id", mInventoryPutawayId);
        this.startActivityForResult(intent, 1);
    }

    private void showEnrollActivity() {
        Intent intent = new Intent(this, EnrollActivity.class);
        intent.putExtra("title", "Enroll Putaway");
        intent.putExtra("m_inventory_putaway_id", mInventoryPutawayId);
        this.startActivityForResult(intent, 2);
    }

    private void deleteHeader() {
        MaterialInventoryPutawayAPI.DeleteHeader(mInventoryPutawayId, new RestfulResponse(this) {
            @Override
            public void onSuccess(org.json.JSONObject response) {
                try {
    ...
    private void deleteDetail(DetailModel detailModel) {
        MaterialInventoryPutawayAPI.DeleteDetail(detailModel, new RestfulResponse(this) {
            @Override
            public void onSuccess(org.json.JSONObject response) {
                try {
                    boolean is_success = false;
                    ...
                    if (!is_success) {
                        ...
                        mInventoryPutawayDetailModel = null;
                        mActivityResultCode = RESULT_OK;
                        mIsDetailViewLoaded = false;
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        if (SessionAPI.IsAllowAccess(mContext, "material/inventory_putaway", "update")) {
            menu.add(Menu.NONE, Menu.FIRST, Menu.NONE, "Edit")
                    .setIcon(R.drawable.edit)
                    .setShowAsAction(MenuItem.SHOW_AS_ACTION_IF_ROOM | MenuItem.SHOW_AS_ACTION_WITH_TEXT);
        }
        if (SessionAPI.IsAllowAccess(mContext, "material/inventory_putaway", "insert")) {
            menu.add(Menu.NONE, Menu.FIRST + 1, Menu.NONE, "Enroll")
                    .setIcon(R.drawable.to_do)
                    .setShowAsAction(MenuItem.SHOW_AS_ACTION_IF_ROOM | MenuItem.SHOW_AS_ACTION_WITH_TEXT);
        }
        if (SessionAPI.IsAllowAccess(mContext, "material/inventory_putaway", "delete")) {
        ...
        switch (id) {
            ...
            case Menu.FIRST + 2:
                ...
                if (currentTabTag.equals("Header")) {
                    ...
                    if (mInventoryPutawayDetailModel != null) {
                        DialogInterface.OnClickListener dialogClickListener = new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                switch (which) {
                                    case DialogInterface.BUTTON_POSITIVE:
                                        deleteDetail(mInventoryPutawayDetailModel);
                                        break;
                                    case DialogInterface.BUTTON_NEGATIVE:
                                    default:
                                        break;
